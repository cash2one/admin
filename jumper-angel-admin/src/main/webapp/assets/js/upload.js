...
$(document).ready(function() {
	...
	$.fn.initOrangeUploader = function() {
		...
		$(this).each(function(){
			...
			+ 'class="img-thumbnail" alt="" onerror="onImgError(this)"/><div id="progressBarWrap_'+id+'" style="display:none;">'
			...
			if (!$(this).next().hasClass("imgPicker")) {
				...
				new plupload.Uploader({
					...
					flash_swf_url : path +'/assets/lib/plupload/Moxie.swf',
					silverlight_xap_url : path + '/assets/lib/plupload//Moxie.xap',
					preinit : {
					...
					init : {
						...
						FileUploaded : function(up, file, info) {
							...
							if (1 == jsonData.code) {
								var fileJSON = eval("(" + jsonData.data + ")");
								upload_img_url = fileJSON.data.fileUrl;
...
function onImgError(obj) {
	$(obj).attr("src", path + "/assets/images/empty.png");
}