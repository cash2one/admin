...
function getPage(keywords){
	$.ajax({
		url:path+"/hospital/doctor/consultant",
		type:"get",
		dataType:"json",
							...
							if (data[i].evaluate==1){
								if(data[i].comments_content!=null){
									if(data[i].comments_status==0){
										html += "<button type='button' class='red_btn_small' onClick='generalize("+data[i].msgId+")' id='"+data[i].msgId+"' value='"+data[i].comments_content+"'>查看评论</button>";
									}else{
